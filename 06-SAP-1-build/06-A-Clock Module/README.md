...
The goal of a clock module is to generate these pulses continuosly. thus going from HIGH > (FALLS) LOW > (RISES) HIGH > (FALLS) LOW > (RISES) HIGH > (FALLS) LOW ............ And every time when state either rises or falls. the computer executes one command. and thus we need these pulses and clock signal to go on continuosly indefinetly so that the computer can keep operating and executing commands.
